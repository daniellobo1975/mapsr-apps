...
# últimas cinco atualizações (em ordem cronológica):
  # 01. inclusão da coleção 08 do Mapbiomas (caminhos e arquivo suporte)
...
caminhos= function(tipo, versao_biomas, ano_referencia, codigo_municipio)
{
  ...
  if (tipo == "arquivos-de-suporte") {caminho= paste0(mnt, "/suporte/")}
  ##  if (tipo == "biomas") {caminho= paste0("/mnt/volume_mapsr_11may22/bioma/", versao_biomas, "/")}
  if (tipo == "biomas") {caminho= paste0(mnt, "downloads/biomas/", versao_biomas, "/tif/")}
  if (tipo == "car-por-municipio") {caminho= paste0(base, "/suporte/car-munics/")}
  if (tipo == "farm-biomas") {caminho= paste0(base, "/fazendas_3000/", versao_biomas, "/", ano_referencia, "/tif/")}
  if (tipo == "principal-output-tif") {caminho= paste0(base, "fazendas_3000/", versao_biomas, "/", ano_referencia, "/tif/")}
  if (tipo == "principal-output-csv") {caminho= paste0(base, "fazendas_3000/", versao_biomas, "/", ano_referencia, "/csv/")}
  if (tipo == "principal-output-json") {caminho= paste0(base, "fazendas_3000/", versao_biomas, "/", ano_referencia, "/json/")}
  if (tipo == "principal-output-geojson") {caminho= paste0(base, "fazendas_3000/", versao_biomas, "/", ano_referencia, "/geoJson/")}
  ...
  if (tipo == "zip-output-temporario") {caminho= paste0(base, "fazendas_3000/", versao_biomas, "/", ano_referencia, "/", codigo_municipio, "/")}
...
correcao_pequenos_poligonos= function(df, tol= 0.1)
{
  
  print("Initiatlizing polygon correction...")
  
  # Extrair apenas POLYGON de GEOMETRYCOLLECTION
  df_cast <- st_collection_extract(df, "POLYGON")
  
  # criar uma linha para cada polígono
  df_cast= st_cast(df_cast, "POLYGON")
  
  # transformá-los em polígonos válidos
  df_cast= st_make_valid(df_cast)
  
  # calcular a área dos polígonos
  df_cast$area_hectares <- as.numeric(st_area(df_cast)) / 10000
  
  # reclassificar os polígonos inferiores à 0.0045 hectares (45m2)
  df_cast_reclassificado <- df_cast %>%
    mutate(
      
      # Alterar 'Legenda' para "Area Florestada" para áreas menores que 0.0045 ha
      Legenda = ifelse(area_hectares < tol, "Area Florestada", Legenda),
      
      # Criar nova coluna indicando se foi reclassificado
      reclassificado = ifelse(area_hectares < tol, "Sim", "Não")
    )
  
  # Selecionar os campos da base original (excluindo a geometria)
  campos_para_agrupamento <- colnames(df)[!colnames(df) %in% c("geometry")]
  
  # Agrupar dinamicamente pelos campos selecionados
  df_unido <- df_cast_reclassificado %>%
    group_by(across(all_of(campos_para_agrupamento))) %>%
    summarise(
      geometry = st_union(geometry), # Unir as geometrias
      .groups = "drop" # Remover agrupamento após o summarise
    )
  
  # Remover agrupamento (opcional)
  df_unido <- ungroup(df_unido)
  
  # garantir que todos os novos polígonos sejam válidos
  df_unido <- st_make_valid(df_unido)
  
  # print("...Finishing polygon correction!")
...
adicionar_lista_de_poligonos= function(bd_geometria)
{
  # Agrupar os dados e armazenar os polígonos em uma lista separada
  df_list= bd_geometria %>% st_collection_extract("POLYGON")
  df_list= df_list %>% filter(st_geometry_type(geometry) %in% c("POLYGON", "MULTIPOLYGON")) 
  df_list= df_list %>% st_cast("POLYGON") %>% st_make_valid()
  df_list <- df_list %>%
    filter(!st_is_empty(geometry)) %>%  # Remove geometrias vazias
    filter(st_geometry_type(geometry) %in% c("POLYGON", "MULTIPOLYGON")) %>%  # Mantém apenas POLYGON/MULTIPOLYGON
    mutate(geometry = st_cast(geometry, "POLYGON", warn = FALSE))
  df_list <- df_list %>% group_by(camada, sub_camada, Legenda) %>% summarise(geometry_list = list(geometry), .groups = "drop")
  df_list$geometry= NULL
  df_list= merge(bd_geometria, df_list, by= c("camada", "sub_camada", "Legenda"))
  df_list$geometry_list <- sapply(df_list$geometry_list, toString)  # Converte lista em string separada por vírgula
  df_list= df_list[c("CAR", "ano", "camada", "Legenda", "latitude", "longitude", "sub_camada", "geometry", "geometry_list")]
...
bioma.completo= function(id.car, fazenda, ano_referencia, versao_biomas, codigo_municipio, verbose, geoJson)
{
  ...
  if (file.exists(pasta.biomas)==FALSE) 
  {
    ...
    if (versao_biomas=="v08") {filename= "/mapbiomas-brazil-collection-80-brasil-"}
    if (versao_biomas=="v09") {filename= "/mapbiomas-brazil-collection-90-brasil-"}
    ...
    ######################
    # salvar TIF  #######
    write_stars(biomas, paste0(caminhos("principal-output-tif", versao_biomas, ano_referencia, ""), id.car, ".tif"))
  ...
  legenda= read_excel(paste0(caminhos("arquivos-de-suporte", versao_biomas, ano_referencia, codigo_municipio), "legendas_", versao_biomas,".xlsx"))
  ...
  # corrigindo pequenos polígonos
  farmbiomas.geoJson= correcao_pequenos_poligonos(farmbiomas.geoJson, tol= 0.1)
  
  # criar lista de polígonos
  farmbiomas.geoJson= adicionar_lista_de_poligonos(farmbiomas.geoJson)
  ...
  bioma= tryCatch(
    {
      ...
      # corrigindo pequenos polígonos
      bioma.parcial= correcao_pequenos_poligonos(bioma.parcial, tol= 0.1)
      ...
      if (geoJson==TRUE)
      {
        ...
        # criar lista de polígonos
        bioma.parcial= adicionar_lista_de_poligonos(bioma.parcial)
...
## busCAR("-22.050656824840956", "-47.96190036699626", "2020", "v09")
