...
# x = function(myData) {
 ...
 remove(verifyFileExist)
  ...
  geojsonFile <- paste0(rootOutput, fileName, ".geojson")
  geo_json_result <- geojson_json(farmappbiomasPolygon)
  # Simplify the geometry information of GeoJSON.
  #geo_json_result_simplify <- ms_simplify(geo_json_result)
  #verify if Exist File
  verifyFileExist <- paste0(rootOutput, fileName, ".geojson")
  if(file.exists(verifyFileExist)){
    file.remove(verifyFileExist)
    file.remove(paste0(rootOutput, fileName, ".geojson"))
  }
 remove(verifyFileExist)
 #Save GeoJson
  geojson <- geojson_write(geo_json_result, file = geojsonFile)
