...
# última atualização: latitude e longitude do centróide das propriedades
...
bioma.completo= function(id.car, fazenda, ano_referencia, versao_biomas, codigo_municipio, verbose, geoJson)
{
  ...
  # atribuir latitude e longitude (agora o erro está aqui -- lat e long diferente dos demais)
  ...
  bioma= tryCatch(
    {
      ...
      if (geoJson==TRUE)
      {
        ...
        # atribuir latitude e longitude para as subcamadas
...
{  
  ...
  car= tryCatch({upload.shapefile(codigo_municipio, "AREA_IMOVEL", versao_biomas, ano_referencia, verbose= "")},
                error= function(e){return(coletor.erros("error", "CC0002", e, c("status", "error_code", "system_error"), df= FALSE))})
  ...
  fazenda= tryCatch({car[car$COD_IMOVEL==id.car,]}, error= function(e){{return(coletor.erros("error", "CC0003", e, c("status", "error_code", "system_error"), df= FALSE))}})
  ...
  farmbiomas= tryCatch({bioma.completo(id.car, fazenda, ano_referencia, versao_biomas, codigo_municipio, "", geoJson)},
                       error= function(e){return(coletor.erros("error", "CC0005", e, c("status", "error_code", "system_error"), df= FALSE))})
