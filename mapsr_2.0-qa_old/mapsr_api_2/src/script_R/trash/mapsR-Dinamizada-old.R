...
# 02. adicionado os polígonos das áreas da fazenda, app, rl e vgn
...
dinamizada_A1_A2= function(arquivo.geojson)
{
  ...
  ## criar A1 e A2 
  a1= app%>% subset(app$Legenda== "Area Nao Florestada") %>% st_intersection(vgn.union)
  a2= app%>% subset(app$Legenda== "Area Florestada") %>% st_intersection(vgn.union)
...
{ 
  ...
  a3= vgn%>% subset(vgn$Legenda== "Area Nao Florestada") %>% st_intersection(vgn.diff)
  a4= vgn%>% subset(vgn$Legenda== "Area Florestada") %>% st_intersection(vgn.diff)
...
dinamizada_A7= function(arquivo.geojson)
{
  ...
  ## dinamizada áreas A7 
  dinamizada.a7= vgn%>% subset(vgn$Legenda== "Area Florestada") %>% st_intersection(vgn.sem.contato)
...
dinamizada_A5_A6= function(arquivo.geojson)
{
  ...
  ## criar R5 e R6 
  a5= app%>% subset(app$Legenda== "Area Nao Florestada") %>% st_intersection(app.dif)
  a6= app%>% subset(app$Legenda== "Area Florestada") %>% st_intersection(app.dif)
