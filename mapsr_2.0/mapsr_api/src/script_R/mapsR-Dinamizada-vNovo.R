...
# Código Principal
##########################################################################################
dinamizar_depcrecated= function(fcar, ano_referencia, versao_biomas)
{
...
dinamizar= function(fcar, ano_referencia, versao_biomas)
{
  ...
  ######################################################
  arquivo.geojson= paste0(caminhos("principal-output-geojson", versao_biomas, ano_referencia, ""), id.car, ".GeoJSON")
  arquivo.tif= paste0(caminhos("principal-output-tif", versao_biomas, ano_referencia, ""), id.car, ".tif")
  arquivo.excel= "/mnt/volume_sfo3_01/suporte/legendas_v08.xlsx"
  
  ##########################################################################################
  # shape para cruzamento com tif
  ##########################################################################################
  # upload do GeoJson disponível no Servidor
  ## mac mini: farm= st_read("/Users/thiagopereiralobo/Documents/Academics/FarmForest/Desenvolvimento/uso da terra/input/SP-3524303-1739C594B4F540C59AF7B47ADBB2D62D.GeoJSON")
  farm= st_read(arquivo.geojson)
  
  # ajustar shape da propriedade
  shape= farm %>% subset(farm$camada == "FZD") # filtro da área da fazenda
  shape= shape %>% st_union() %>% st_as_sf() # unir área florestada e não florestada
  shape$CAR= farm$CAR[1]
  
  ##########################################################################################
  # arquivo tif
  ##########################################################################################
  # upload arquivo em tif
  tif.file= read_stars(arquivo.tif)
  
  # ajustar arquivo tiff
  tif.file= st_as_sf(tif.file) # transformar em objeto simple features
  names(tif.file)[1]= "bioma_code" # renomear columa de interesse
  
  ##########################################################################################
  # arquivo contendo legendas
  ##########################################################################################
  legenda= read_excel(arquivo.excel)
  
  # removendo coluna desnecessária de levenda
  legenda <- legenda %>% select(-Legenda)
  
  # renomear campo para ficar tratável
  names(legenda)[2]= "legenda_detalhada" # renomear campo de interesse
  
  
  ##########################################################################################
  # criar o farmbiomas
  ##########################################################################################
  # cruzamento entre tif e shape
  farmbiomas= st_intersection(tif.file, shape)
  
  # cruzamento farmbiomas com legenda
  farmbiomas= merge(farmbiomas, legenda, by= "bioma_code")
  
  ##########################################################################################
  # base de dados uso da terra
  ##########################################################################################
  # agrupar farmbiomas por tipo de legenda
  uso_da_terra= farmbiomas %>%  group_by(bioma_code, legenda_detalhada)  %>% summarise()
  
  # renomeando o campo legenda para fazer match com a base GeoJson
  names(uso_da_terra)[2]= "Legenda"
  
  # adicionando outros campos de interesse
  uso_da_terra$CAR= farm$CAR[1]
  uso_da_terra$ano= farm$ano[1]
  uso_da_terra$camada= "FZD-LU"
  uso_da_terra$sub_camada= "N/a"
  uso_da_terra$latitude= farm$latitude[1]
  uso_da_terra$longitude= farm$longitude[1]
  
   
  ##########################################################################################
  # unificando com base GeoJson (para base única)
  ##########################################################################################
  # reordenando campos antes de fazer o bind rowos
  uso_da_terra= uso_da_terra[c("CAR", "ano", "camada", "Legenda", "latitude", "longitude", "sub_camada", "geometry")]
  
  # bind rows (base GeoJson final)
  farm= bind_rows(farm, uso_da_terra)
  
  # calcular áreas em termos de hectare
  farm$area_ha= as.numeric(st_area(farm))/10000

  # reordenando campos antes de fazer o bind rowos
  farm= farm[c("CAR", "ano", "camada", "Legenda", "latitude", "longitude", "sub_camada", "geometry", "area_ha")]
  
  # substituir arquivo geojson
  salvar.geoJson(arquivo.geojson, farm, "FZD")
