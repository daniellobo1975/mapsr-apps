...
Paths= function(folder)
{
  ...
  if (folder=='unzipped-files') {path= '/mnt/volume_mapsr_11may22/consulta/crediguacu-2023/arquivos'}
...
farm.land.use= function(ncar, data.farms, ano.biomas, land.type= "imovel")
{
  # identify the municipality
  munic= substring(ncar, first= 4, last= 10)
  
  # upload AREA IMOVEL SICAR
  farm= st_read(paste0(Paths('crediguacu'), '/sicar-temp/zip-level-2/IMOVEL/AREA_IMOVEL.shp'),
                quiet= TRUE,
  ...
  if (land.type!="imovel")
  {
    ...
    # upload app
    app= st_read(paste0(Paths('crediguacu'), '/sicar-temp/zip-level-2/', fname, '/', fname,'.shp'),
                 quiet= TRUE,
                 options = "ENCODING=UTF-8")
...
for (ano.biomas in c("2018", "2019", "2020", "2021"))
{
  for (land.type in c("app", "rl", "imovel"))
  {
    ...
    # set file to loop
    car.list= read.csv(paste0(Paths('crediguacu'), '/car_list.csv'), sep = ',')
    ...
    for (car in car.list$fcar)
    {
      ...
      # del used folder
      unlink(paste0(Paths('crediguacu'), "/sicar-temp"), recursive = TRUE)
    ...
    # save
    write.csv(farms, paste0(Paths('crediguacu'), '/', outfile))
